...
from buffer import PriorityRolloutStorage
...
class CnnDQNPERAgent:
    def __init__(self, config: Config):
        self.config = config
        self.is_training = True
        self.buffer = PriorityRolloutStorage(config)
    ...
    def learning(self, fr):
        # @change
        idxs, s0, s1, a, r, done = self.buffer.sample(self.config.batch_size)
        if self.config.use_cuda:
            s0 = s0.float().to(self.config.device)/255.0 # [32, 12, 84, 84]
            s1 = s1.float().to(self.config.device)/255.0 # [32, 12, 84, 84]
            a = a.to(self.config.device) # [32, 1]
            r = r.to(self.config.device) # [32, 1]
            done = done.to(self.config.device) # [32, 1]
        ...
        # q_values = self.model(s0).cuda() # [32, 6]

        # How to calculate argmax_a Q(s,a)
        # actions = q_values.max(1)[1] # [32] 得分最高的动作 torch.max()返回tuple indices和values
        ...
        # @change
        td_errs = (q_eval - q_target).cpu().squeeze().tolist()
        self.buffer.update(idxs, td_errs)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--algo', default='per', type=str, help='algo type')
    ...
    config.max_buff = 100000 # buffer_size
    config.learning_interval = 4
    config.update_tar_interval = 1000
    config.batch_size = 32 # batch_size = 32
    ...
    print(config.action_dim, config.state_shape) # 6 (12, 84, 84)
    agent = CnnDQNPERAgent(config)
